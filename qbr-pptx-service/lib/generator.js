...
const TEXT_REPLACEMENTS = [
  [/√É‚Äö√Ç¬£|√É∆í√Ü‚Äô√É¬¢√¢‚Äö¬¨√Ö¬°√É∆í√¢‚Ç¨≈°√É‚Äö√Ç¬£|√É¬ê√¢‚Ç¨≈ì√É¬¢√¢‚Äö¬¨√Ö¬°√É¬ê√¢‚Ç¨‚Ñ¢√É¬ê√ã‚Ä†|√É¬ê√ã‚Ä†/g, "\u00A3"],
  [/√É‚Äö√¢‚Äö¬¨|√É∆í√Ü‚Äô√É‚Äö√Ç¬¢√É¬¢√¢‚Ç¨≈°√Ç¬¨√É‚Ä¶√Ç¬°√É∆í√¢‚Ç¨≈°√É‚Äö√Ç¬¨|√É¬ê√¢‚Ç¨≈ì√É¬¢√¢‚Äö¬¨√Ö¬°√É¬ê√¢‚Ç¨‚Ñ¢√É‚Äö√Ç¬§/g, "\u20AC"],
  [/√É‚Äö√Ç¬•/g, "\u00A5"],
  [/√É‚Äö/g, ""],
  [/√É‚Äö√Ç¬†/g, " "],
  [/√É¬¢√¢‚Äö¬¨√¢‚Ç¨≈ì|√É¬¢√¢‚Äö¬¨√¢‚Ç¨¬ù/g, "-"],
  [/√É¬¢√¢‚Äö¬¨√ã≈ì|√É¬¢√¢‚Äö¬¨√¢‚Äû¬¢/g, "'"],
  [/√É¬¢√¢‚Äö¬¨√Ö‚Äú|√É¬¢√¢‚Äö¬¨√Ø¬ø¬Ω/g, '"'],
  [/z√É‚Ä¶√¢‚Ç¨≈°|z√É∆í√Ü‚Äô√É¬¢√¢‚Äö¬¨√Ç¬¶√É∆í√Ç¬¢√É¬¢√¢‚Ç¨≈°√Ç¬¨√É‚Ä¶√Ç¬°/g, "z\u0142"]
];
...
const MOJIBAKE_MARKER_PATTERN = /(?:√|¬|‚|≈[º∫õöÇ]|ƒ[ÖôáÑ]|√É|√¢|√Ö|√Ñ)/;
const MOJIBAKE_MARKER_GLOBAL_PATTERN = /(?:√|¬|‚|≈[º∫õöÇ]|ƒ[ÖôáÑ]|√É|√¢|√Ö|√Ñ)/g;
...
const UI_LABELS_BY_LANGUAGE = {
  FR: {
    ...
    currentPeriod: "PÈriode actuelle",
    comparisonPeriodYoy: "PÈriode de comparaison (YoY)",
    basisYoy: "RÈfÈrence : glissement annuel (YoY)",
    publisherActivityBySegment: "ActivitÈ des Èditeurs par segment",
    keyObservations: "Observations clÈs",
    reportingPeriodPrefix: "PÈriode de reporting",
    dataAsOfPrefix: "DonnÈes au",
    comparisonPeriodPrefix: "PÈriode de comparaison",
    allFiguresStatement: "Toutes les valeurs sont prÈsentÈes en {currency}, sauf indication contraire. La variation YoY est calculÈe entre la pÈriode actuelle et la pÈriode de comparaison.",
  ...
  IT: {
    ...
    allFiguresStatement: "Tutti i valori sono riportati in {currency}, salvo diversa indicazione. La variazione YoY Ë calcolata come periodo corrente vs periodo di confronto.",
    analysisTagSuffix: "Analisi"
  },
  NO: {
    qbrReport: "QBR-rapport",
    anyQuestions: "Sp¯rsmÂl?",
    ...
    basisYoy: "Grunnlag: Âr-over-Âr (YoY)",
  ...
  SV: {
    qbrReport: "QBR-rapport",
    anyQuestions: "NÂgra frÂgor?",
    kpiComparisonTemplate: "{current} mot {previous} fg. Âr",
    thankYouSubtitleTemplate: "TD affiliateprogram - {period} kvartalsgenomgÂng",
    currentPeriod: "Aktuell period",
    comparisonPeriodYoy: "J‰mfˆrelseperiod (YoY)",
    basisYoy: "Grund: Âr ˆver Âr (YoY)",
    ...
    dataAsOfPrefix: "Data per",
    comparisonPeriodPrefix: "J‰mfˆrelseperiod",
    allFiguresStatement: "Alla siffror rapporteras i {currency} om inget annat anges. YoY-variansen ber‰knas som aktuell period j‰mfˆrt med j‰mfˆrelseperiod.",
    analysisTagSuffix: "Analys"
  },
  DA: {
    qbrReport: "QBR-rapport",
    anyQuestions: "Nogen sp¯rgsmÂl?",
    kpiComparisonTemplate: "{current} mod {previous} sidste Âr",
    thankYouSubtitleTemplate: "TD affiliateprogram - {period} kvartalsgennemgang",
    currentPeriod: "Aktuel periode",
    comparisonPeriodYoy: "Sammenligningsperiode (YoY)",
    basisYoy: "Grundlag: Âr-til-Âr (YoY)",
  ...
  ES: {
    ...
    anyQuestions: "øPreguntas?",
    kpiComparisonTemplate: "{current} vs {previous} aÒo ant.",
    thankYouSubtitleTemplate: "Programa de afiliaciÛn TD - {period} RevisiÛn trimestral",
    currentPeriod: "PerÌodo actual",
    comparisonPeriodYoy: "PerÌodo de comparaciÛn (YoY)",
    ...
    reportingPeriodPrefix: "PerÌodo del informe",
    dataAsOfPrefix: "Datos a fecha de",
    comparisonPeriodPrefix: "PerÌodo de comparaciÛn",
    allFiguresStatement: "Todas las cifras se presentan en {currency}, salvo que se indique lo contrario. La variaciÛn YoY se calcula como perÌodo actual frente a perÌodo de comparaciÛn.",
  ...
  PL: {
    ...
    comparisonPeriodYoy: "Okres porÛwnawczy (r/r)",
    basisYoy: "Podstawa: rok do roku (r/r)",
    publisherActivityBySegment: "Aktywnosc wydawcÛw wedlug segmentu",
    keyObservations: "Kluczowe obserwacje",
    reportingPeriodPrefix: "Okres raportowania",
    dataAsOfPrefix: "Dane na dzien",
    comparisonPeriodPrefix: "Okres porÛwnawczy",
    allFiguresStatement: "Wszystkie wartosci raportowane sa w walucie {currency}, o ile nie wskazano inaczej. Zmiana r/r jest liczona jako biezacy okres wzgledem okresu porÛwnawczego.",
    analysisTagSuffix: "Analiza",
    segmentSignalUnavailable: "Sygnal segmentu jest niedostepny.",
    detailedMovementUnavailable: "SzczegÛlowy opis zmian nie jest dostepny w tym wyciagu.",
    kpiSignalGeneric: "Sygnal KPI",
    kpiDriverUnavailable: "Brak potwierdzonego czynnika na podstawie dostepnych danych KPI.",
    kpiDetailUnavailable: "SzczegÛly nie sa dostepne w biezacym wyciagu.",
    kpiTitleConversionRateImprovement: "Poprawa wspÛlczynnika konwersji",
...
function cellTextColor(table, column, value, deck, row = [], cellIndex = 0) {
  ...
  const signedDeltaValue = /^[+\-]/.test(text) || text.startsWith("‚ñ≤") || text.startsWith("‚ñº");
